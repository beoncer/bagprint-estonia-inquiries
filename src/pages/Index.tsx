...
const Index = () => {
  ...
  const defaultHeroContent = {
    title: "Kvaliteetsed kotid ja pakendid teie brändile",
    description: "Leatex pakub laia valikut puuvillakotte, paberkotte, paelaga kotte ja e-poe pakendeid, mida saab kohandada teie brändi logo ja disainiga.",
    button1: { value: "Vaata tooteid", link: "/tooted" },
    button2: { value: "Küsi pakkumist", link: "/paring" }
  };

  const defaultBannerUrl = "/lovable-uploads/df14f86d-deb5-425a-bbf5-22630946d650.png";

  // Use default content immediately, then update when data loads
  const displayHeroContent = isHeroLoading ? defaultHeroContent : (heroContent || defaultHeroContent);
  const displayBannerUrl = isBannerLoading ? defaultBannerUrl : (bannerUrl || defaultBannerUrl);

  // Add cache buster to banner URL
  const bannerUrlWithCacheBuster = `${displayBannerUrl}?t=${Date.now()}`;
      ...
      <section className="py-6">
        <div className="max-w-screen-2xl mx-auto w-full px-4 md:px-8 xl:px-20">
          <div 
            ...
            style={{
              backgroundImage: `url('${bannerUrlWithCacheBuster}')`,
              backgroundSize: "cover",
              backgroundPosition: "center",
              height: "auto",
              minHeight: "520px"
            }}
            key={bannerUrlWithCacheBuster} // Force re-render when URL changes
          >
                  ...
                  <div className="flex flex-col h-full justify-between">
                    <div className="mt-4 md:mt-8">
                      <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold mb-4 md:mb-6">
                        {displayHeroContent?.title}
                      </h1>
                      <p className="text-base sm:text-lg md:text-xl text-gray-600 mb-6">
                        {displayHeroContent?.description}
                      </p>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-3 md:gap-4 mt-6 mb-6 md:mb-10">
                      <Button 
                        ...
                        asChild
                      >
                        <Link to={displayHeroContent?.button1?.link || "/tooted"}>
                          {displayHeroContent?.button1?.value || "Vaata tooteid"}
                        </Link>
                      </Button>
                      <Button 
                        variant="white" 
                        ...
                        asChild
                      >
                        <Link to={displayHeroContent?.button2?.link || "/paring"}>
                          {displayHeroContent?.button2?.value || "Küsi pakkumist"}
                        </Link>
                      </Button>
                    </div>
                  </div>
