...
const Breadcrumb: React.FC<BreadcrumbProps> = ({ items, className = "" }) => {
  ...
  if (breadcrumbItems.length === 0 || (loading && (context.pageType === 'product-detail' || context.pageType === 'blog-post'))) {
    return loading ? (
      <nav className={`flex items-center space-x-1 text-sm text-gray-600 font-bold ${className}`} aria-label="Breadcrumb">
  ...
  return (
    <nav className={`flex items-center space-x-1 text-sm text-gray-600 font-bold ${className}`} aria-label="Navigatsioon">
      <Link 
        to="/" 
